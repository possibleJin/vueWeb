  ...
  export default {
    ...
    computed: {
      ...
      ...mapState({
        ...
        callingInfo: state => state.callingInfo,
        userID: state => state.userID,
        userSig: state => state.userSig,
        videoRoom: state => state.videoRoom,
    ...
    watch: {
      ...
      // 值第一次绑定的时候，不会执行监听函数，只有值发生改变才会执行，需要在最初绑定值的时候也执行函数，则就需要用到immediate属性
      callingUserList: {
        handler(newValue) {
