...
export default {
  ...
  computed: {
    ...mapState({
      ...
      groupLiveInfo: state => state.groupLiveInfo
    }),
    anchorAvatar() {
  ...
  methods: {
    ...
    async createRoom() {
      ...
      // axios (`https://service-62h5r0ea-1252463788.gz.apigw.tencentcs.com/release/forTestAdvanced?method=createRoom&appId=1253985742&type=liveRoom&title=${this.roomName}&anchorId=${this.user.userID}`)
      // await axios (`https://service-62h5r0ea-1252463788.gz.apigw.tencentcs.com/release/forTestAdvanced?method=createRoom&appId=${this.user.sdkAppID}&type=groupLive&title=${this.roomName}&anchorId=${this.user.userID}&roomId=${this.roomID}`)
      this.$store.commit('updateGroupLiveInfo', { roomID: this.roomID, roomName: this.roomName })
      this.createGroupLiveAvChatRoom()
      // var res = await axios (`https://service-62h5r0ea-1252463788.gz.apigw.tencentcs.com/release/forTestAdvanced?method=getRoomList&appId=1253985742&type=liveRoom`)
      var res = await axios (`https://service-62h5r0ea-1252463788.gz.apigw.tencentcs.com/release/forTestAdvanced?method=getRoomList&appId=appId=${this.user.sdkAppID}&type=liveRoom`)
      console.error(res,'kk')
    ...
    async startPushStream() {
      ...
      // const livedomainname = 'tuikit.qcloud.com'
      const livedomainname = '99592.liveplay.erickyan.com'
      const url = `room://livedomainname=${livedomainname}&sdkappid=${this.user.sdkAppID}&roomid=${this.roomID}&userid=${this.user.userID}&usersig=${encodeURIComponent(this.user.userSig)}&streamid=${streamID}`
